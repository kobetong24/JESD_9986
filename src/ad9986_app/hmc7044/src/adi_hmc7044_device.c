...
int32_t adi_hmc7044_device_chip_id_get(adi_hmc7044_device_t *device, adi_cms_chip_id_t *chip_id)
{
    ...
    /* Registers 0x078-0x07A form the HMC7044 chip identity:
     *   0x078 -> device revision
     *   0x079 -> product id low byte
     *   0x07A -> product id high byte
     * The HMC7044 exposes no separate chip-type register, so chip_type is
     * defined explicitly here (rather than left to the caller) to ensure the
     * returned structure is fully populated. */
    chip_id->chip_type = 0;
